...
header("Cache-Control: post-check=0, pre-check=0", false);     	
	if($_POST['viewData']==1){ // เงื่อนไขกรณีส่งค่ามาครั้งแรก แสดงรายการทั้งหมดที่มีอยู่ 
		...
		(
			(user1='".$_SESSION['ses_user_id']."' AND user2='".$_POST['userID']."') OR 
			(user1='".$_POST['userID']."' AND user2='".$_SESSION['ses_user_id']."')
		)	
		ORDER BY id 
		";
	...
	if($result){
		while($row = $result->fetch_array()){
			$json_data[]=array(
				"max_id"=>$row['id'],
				"data_align"=>($_SESSION['ses_user_id']==$row['user1'])?"right":"left",// ถ้าเป็นข้อความที่ส่งจากผู้ใช้ขณะนั้น
				"data_msg"=>$row['msg']			
			);	
		}
	}
