<?php
    ...
    $sql = "SELECT m.id as id,m.market as market FROM market m INNER JOIN gmarket g on m.id = g.marketid 
            where g.username = '$username'";
    $queryA = mysqli_query($objCon, $sql);

    $Profilesql = "SELECT p.name_surname as name_surname,p.address as address,p.subdictrict as subdictrict,d.district_name 
                   as district_name,d.district_id as district_id,p.phone as phone,p.facebook as facebook,p.line 
                   as line,p.email as email, p.brand as brand,p.farmer_group as farmer_group, p.link_youtube 
                   as link_youtube,p.username as username,p.latitude as latitude,p.longitude as longitude 
                   FROM profile p INNER join districts d on p.district_id = d.district_id  
                   WHERE p.id = '$id'";
